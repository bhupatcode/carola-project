...
if (isset($_POST['reset'])) {
    ...
    if ($count == 0) {
        ...
        $stmt = $conn->prepare("SELECT aid FROM admin WHERE areset_token = ? AND areset_expiry > NOW()");
        ...
        if ($stmt->affected_rows > 0) {
            ...
            $stmt = $conn->prepare("UPDATE admin SET apassword = ?, areset_token = NULL , areset_expiry=NOW()  WHERE areset_token = ?");
