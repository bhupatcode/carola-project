...
<head>
    ...
    <style>
        ...
        a.change-btn {
            display: inline-block;
            margin-top: 15px;
            background-color:rgb(50, 172, 193);
            color: white;
            padding: 12px 25px;
            text-decoration: none;
            border-radius: 8px;
...
<body>
    <div class="booking-details-container">
        ...
        <div class="booking-info">
            <img src="../admin/<?php echo explode(',', $row['image'])[0]; ?>" alt="Car Image" class="booking-img">
            ...
            <div class="detail">
                ...
                <div class="info-box">
                    <p><strong>Status:</strong> <span class="<?php
                                                                echo ($row['status'] == 0) ? 'status-in-progress' : (($row['status'] == 1) ? 'status-success' : (($row['status'] == 2) ? 'status-rejected' : 'status-Returned'));
                                                                ?>">
                            <?php
                            echo ($row['status'] == 0) ? 'In Progress' : (($row['status'] == 1) ? 'Success' : (($row['status'] == 2) ? 'Rejected' : ' Returned'));
                            ?></span></p>
                </div>
                 <?php  if ($row['status']==2) { ?>
                <div class="info-box">
    ...
    <?php if ($row['status'] != 2 && $row['status'] !=3) { ?>
        <a href="../admin/Approve.php?canid=<?php echo $row['bookingno'] ?> && vid=<?php echo $row['vid']; ?>  && userEmail=<?php echo $row['userEmail']; ?>" class="cancel-btn" onclick="return confirm('Do you really want to cancel your booking?');"> Cancel Booking</a>
        <a href="modify_booking.php?id=<?php echo $row['id']; ?>" class="change-btn" onclick="return confirm('Do you really change booking?');"> Change Booking Date</a>
    <?php  } ?>
    <?php 
    ...
    if ($row['status'] == 3 && $result==0) { ?>
        <a href="feedback.php?vid=<?php echo $row['vid']; ?>" class="cancel-btn"> Feedback To The Car</a>
    <?php  } ?>
    </div>
</body>
